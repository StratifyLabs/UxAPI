...
#include "ux/sgfx/FileFont.hpp"

using namespace ux::sgfx;

FileFont::FileFont() { m_kerning_pairs = 0; }

FileFont::FileFont(const var::String &name, FileLocation offset) {
  m_kerning_pairs = 0;
...
int FileFont::set_file(const var::String &path, FileLocation offset) {
  u32 pair_size;
  ...
  if (
    m_file.read(offset, &m_header, fs::File::Size(sizeof(m_header)))
    != sizeof(m_header)) {
    set_error_number(m_file.error_number());
    return -1;
  }
...
int FileFont::load_char(sg_font_char_t &ch, char c, bool ascii) const {
  ...
  offset = m_offset + sizeof(sg_font_header_t)
           + sizeof(sg_font_kerning_pair_t) * m_header.kerning_pair_count
           + ind * sizeof(sg_font_char_t);
  if (
    (ret
     = m_file.read(fs::File::Location(offset), &ch, fs::File::Size(sizeof(ch))))
    != sizeof(ch)) {
    return -1;
  }
...
int FileFont::load_kerning(u16 first, u16 second) const {
  ...
  for (i = 0; i < kerning_count; i++) {
    if (
      (m_kerning_pairs[i].unicode_first == first)
