...
void LineGraph::draw(const DrawingScaledAttributes &attributes) {
  ...
  bitmap.set_pen(sgfx::Pen().set_color(theme()->background_color()))
    .draw_rectangle(attributes.region());

  const sgfx::Region region_inside_margin
  ...
  const sgfx::Region plot_region = calculate_region_inside(
    region_inside_margin,
    {0,
     0,
     x_axis().is_valid() ? static_cast<u8>(effective_axes_area.height())
                         : static_cast<u8>(0),
     y_axis().is_valid() ? static_cast<u8>(effective_axes_area.width())
                         : static_cast<u8>(0)});
  ...
  if (y_axis().is_valid()) {
    ...
    bitmap.draw_line(
      plot_region.point(),
      plot_region.point() + Point(0, plot_region.height()));
  }
  ...
  if (x_axis().is_valid()) {
    ...
    bitmap.draw_line(
      plot_region.point() + Point(0, plot_region.height()),
      plot_region.point() + Point(plot_region.width(), plot_region.height()));
  }
  ...
  for (const auto &data : *data_set()) {
    ...
    bitmap.set_pen(Pen().set_color(color));

    for (const auto &point : data) {
      // map point to a place on the chart and draw it
      if (is_first_point) {
        is_first_point = false;
        cursor = transform_point(plot_region, point);
      } else {
        Point next_point = transform_point(plot_region, point);

        bitmap.draw_line(cursor, next_point);
        cursor = next_point;
      }
    }
  }
